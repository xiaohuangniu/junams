<?php
namespace qiniu\Qiniu\Storage;

use qiniu\Qiniu\Auth;
use qiniu\Qiniu\Config;
use qiniu\Qiniu\Zone;
use qiniu\Qiniu\Http\Client;
use qiniu\Qiniu\Http\Error;
...
final class BucketManager
{
    ...
    public function listFiles($bucket, $prefix = null, $marker = null, $limit = 1000, $delimiter = null)
    {
        ...
        \qiniu\Qiniu\setWithoutEmpty($query, 'prefix', $prefix);
        \qiniu\Qiniu\setWithoutEmpty($query, 'marker', $marker);
        \qiniu\Qiniu\setWithoutEmpty($query, 'limit', $limit);
        \qiniu\Qiniu\setWithoutEmpty($query, 'delimiter', $delimiter);
    ...
    {
        $path = '/stat/' . \qiniu\Qiniu\entry($bucket, $key);
    ...
    {
        $path = '/delete/' . \qiniu\Qiniu\entry($bucket, $key);
    ...
    {
        $from = \qiniu\Qiniu\entry($from_bucket, $from_key);
        $to = \qiniu\Qiniu\entry($to_bucket, $to_key);
        $path = '/copy/' . $from . '/' . $to;
        list(, $error) = $this->rsPost($path);
        return $error;
    }
    ...
    {
        $resource = \qiniu\Qiniu\entry($bucket, $key);
        $encode_mime = \qiniu\Qiniu\base64_urlSafeEncode($mime);
    ...
    public function fetch($url, $bucket, $key = null)
    {
        ...
        $resource = \qiniu\Qiniu\base64_urlSafeEncode($url);
        $to = \qiniu\Qiniu\entry($bucket, $key);
    ...
    private static function oneKeyBatch($operation, $bucket, $keys)
    {
        $data = array();
        foreach ($keys as $key) {
            array_push($data, $operation . '/' . \qiniu\Qiniu\entry($bucket, $key));
        }
        return $data;
    }
    ...
    private static function twoKeyBatch($operation, $source_bucket, $key_pairs, $target_bucket)
    {
        ...
        foreach ($key_pairs as $from_key => $to_key) {
            $from = \qiniu\Qiniu\entry($source_bucket, $from_key);
            $to = \qiniu\Qiniu\entry($target_bucket, $to_key);
