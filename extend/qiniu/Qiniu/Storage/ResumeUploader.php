...
final class ResumeUploader
{
    ...
    public function upload()
    {
        ...
        while ($uploaded < $this->size) {
            ...
            $crc = \qiniu\Qiniu\crc32_data($data);
    ...
    private function fileUrl()
    {
        ...
        $url .= '/mimeType/' . \qiniu\Qiniu\base64_urlSafeEncode($this->mime);
        if ($this->key != null) {
            $url .= '/key/' . \qiniu\Qiniu\base64_urlSafeEncode($this->key);
        }
        if (!empty($this->params)) {
            foreach ($this->params as $key => $value) {
                $val =  \qiniu\Qiniu\base64_urlSafeEncode($value);
                $url .= "/$key/$val";
            }
        }
        return $url;
