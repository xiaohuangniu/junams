...
class Kd100{
    ...
    public function Obtain($order='', $type=''){
        ...
        # 自动识别快递类型
        if (empty($type)) {
            $res  = json_decode($this->https_request('https://www.kuaidi100.com/autonumber/autoComNum?text='.$order), true);
            if (!$res || empty($res['auto'])) return $this->returnEcho('02', '快递类型自动识别失败，单号可能已过期');
            $type = $res['auto'][0]['comCode'];
        }else{
            $type = $this->typeGet($type);
            if (!$type) return $this->returnEcho('03', '暂无该快递公司类型');
        }
        $res = json_decode($this->https_request('https://www.kuaidi100.com/query?type='.$type.'&postid='.$order), true);
        if (!$res || !$res['data']) return $this->returnEcho('04', $res['message']);
